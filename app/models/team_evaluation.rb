class TeamEvaluation < ActiveRecord::Base
  ...
  def self.send_all_team_evaluations(iteration_id)
    ...
    team_evaluations = self.select("distinct on (grader_id, gradee_id) grader_id, gradee_id, id, created_at").where("iteration_id = #{iteration_id}").order("grader_id desc, gradee_id desc, created_at desc").includes([:gradee])
